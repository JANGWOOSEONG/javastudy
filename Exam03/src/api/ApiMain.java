...
public class ApiMain{
	...
	public static void main(String[] args) {
		try {
			...
			apiURL += "?serviceKey=" + URLEncoder.encode("+f/VvIFrOCH3yNpYRLtlzkRhGrcHJMgAdMOQtg==", "UTF-8");
			...
			for(int i = 0; i < itemList.length(); i++) {
				...
				accident.setOccrrncDt(occrrncDt);       // 발생월일시
				accident.setOccrrncDayCd(occrrncDayCd); // 발생요일코드
				accident.setDthDnvCnt(dthDnvCnt);       // 사망자수
				accident.setInjpsnCnt(injpsnCnt);       // 부상자수
				//객체를 ArrayList에 저장한다.
				list.add(accident);
